  ...
  "sections": [
    ...
    },
    {
      ...
      "paragraphs": [
        ...
        },
        {
          ...
          "equations": [
            {
              "id": "eq-01.02-p005-radial-probability",
              "label": "",
        ...
        {
          "id": "01.02-p010",
          "text": "An alternative family is the Gaussian-type orbital,\n\\[\n\\chi(x,y,z)\\propto x^i y^j z^k \\exp(-\\alpha r^2)\n\\tag{1.5}\n\\]\nwhere the nonnegative integers \\(i\\), \\(j\\), and \\(k\\) encode the angular character. When \\(i+j+k=0\\), one has an \\(s\\)-type Gaussian; when the sum is 1, a \\(p\\)-type function; when it is 2, a \\(d\\)-type one. Gaussian orbitals are less faithful than STOs near the nucleus because they are too smooth there: they lack the cusp behavior of the exact hydrogenic and Slater forms. Yet they are vastly easier to integrate in large ab initio calculations. For that reason, practical basis sets often approximate a Slater-like shape by a contracted sum of Gaussians,\n\\[\n\\exp(-\\zeta r)\\approx c_1\\exp(-\\alpha_1 r^2)+c_2\\exp(-\\alpha_2 r^2)+\\cdots\n\\tag{1.6}\n\\]\nThis is the origin of labels such as STO-3G: three primitive Gaussians are combined to mimic one Slater-type function. A basis called 3-21G uses three contracted Gaussians for the core and a split-valence description for the outer region; 6-311G** goes further, using six primitives for the core, a triple-split valence shell, and polarization functions on both heavy atoms and hydrogen. There is no universally best basis. One chooses among these families according to the balance one needs between chemical realism, computational cost, and the kind of property one hopes to calculate.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.5",
    ...
    },
    {
      ...
      "paragraphs": [
        {
          "id": "01.03-p001",
          "text": "For a molecule built from \\(m\\) atomic basis functions \\(\\{\\chi_1,\\chi_2,\\ldots,\\chi_m\\}\\), one seeks \\(m\\) molecular orbitals. Before combining them, each basis function is normalized so that\n\\[\n\\int \\chi_\\mu^*(\\mathbf r)\\chi_\\mu(\\mathbf r)\\,d\\tau =1\n\\tag{1.7}\n\\]\nIn bra-ket notation this is\n\\[\n\\langle \\chi_\\mu|\\chi_\\mu\\rangle=1\n\\tag{1.8}\n\\]\nThe notation does not add new physics; it simply compresses the integral into a form that is easier to manipulate once many orbitals and many matrix elements appear.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.7",
        ...
        {
          "id": "01.03-p002",
          "text": "The linear-combination step writes each molecular orbital as\n\\[\n\\psi_i=\\sum_\\mu c_{\\mu i}\\chi_\\mu\n\\tag{1.9}\n\\]\nThe coefficients \\(c_{\\mu i}\\) tell how much of atomic orbital \\(\\mu\\) enters molecular orbital \\(i\\), and their signs matter because wavefunctions can reinforce or cancel. The resulting molecular orbitals are required to satisfy orthonormality,\n\\[\n\\langle \\psi_i|\\psi_j\\rangle=\\delta_{ij}\n\\tag{1.10}\n\\]\nThis already hides a simplifying move. The molecule is being described inside a finite basis rather than in the full infinite space of all possible one-electron functions. Within that chosen space, however, the problem becomes concrete: one need only determine the coefficient patterns that give the best orbitals.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.9",
        ...
        {
          "id": "01.03-p003",
          "text": "Those coefficients are not chosen by hand. One introduces an effective one-electron Hamiltonian \\(H^\\mathrm{eff}\\) and asks for orbitals satisfying\n\\[\nH^\\mathrm{eff}\\psi_i=e_i\\psi_i\n\\tag{1.11}\n\\]\nThe word effective is crucial. The exact electronic problem is a many-electron problem, with each electron feeling all the nuclei and all the others at once. Molecular orbital theory replaces that full many-body problem by a one-electron operator that represents an average field. The orbital energy is then the expectation value\n\\[\ne_i=\\frac{\\langle \\psi_i|H^\\mathrm{eff}|\\psi_i\\rangle}{\\langle \\psi_i|\\psi_i\\rangle}\n\\tag{1.12}\n\\]\nwhich reduces to\n\\[\ne_i=\\langle \\psi_i|H^\\mathrm{eff}|\\psi_i\\rangle\n\\tag{1.13}\n\\]\nonce the orbital is normalized.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.11",
        ...
        {
          "id": "01.03-p004",
          "text": "Given two atomic orbitals on different centers, the overlap integral is\n\\[\nS_{\\mu\\nu}=\\langle \\chi_\\mu|\\chi_\\nu\\rangle\n\\tag{1.14}\n\\]\nIt measures not just spatial proximity but signed proximity: lobes of the same phase contribute positively, while opposite phases contribute negatively. The small diagrams 1.6-1.10 make this visible. If one flips the phase of one orbital, the overlap changes sign, as in\n\\[\n\\langle \\chi_\\mu|{-}\\chi_\\nu\\rangle=-S_{\\mu\\nu}\n\\tag{1.15}\n\\]\nif both phases are flipped, the original sign returns,\n\\[\n\\langle {-}\\chi_\\mu|{-}\\chi_\\nu\\rangle=S_{\\mu\\nu}\n\\tag{1.16}\n\\]\nThis is why the shading convention introduced earlier is more than a drawing convenience. It lets one read the sign of an overlap directly from geometry.",
          "visuals": [
            {
              "id": "diag-1.6",
              "label": "Diagram 1.6",
        ...
        {
          "id": "01.03-p005",
          "text": "The size of the overlap controls how strongly two orbitals can interact. Figure 1.4 organizes the main qualitative types. When there are no nodes along the internuclear axis one has \\(\\sigma\\) overlap; one nodal plane containing that axis gives \\(\\pi\\) overlap; two such nodal planes give \\(\\delta\\) overlap. Each extra node suppresses constructive interpenetration, so for comparable valence orbitals in the same period one usually finds\n\\[\n\\sigma>\\pi>\\delta\n\\]\nin magnitude. This is only a rule of thumb. Radial extent, principal quantum number, electronegativity, and above all internuclear distance can reverse naive expectations. The point is not that nodal count alone decides everything, but that it gives the first qualitative estimate of how much two orbitals can communicate.",
          "visuals": [
            {
              "id": "fig-1.4",
              "label": "Figure 1.4",
        ...
        {
          "id": "01.03-p007",
          "text": "The energetic counterpart of overlap is the interaction integral\n\\[\nH_{\\mu\\nu}=\\langle \\chi_\\mu|H^\\mathrm{eff}|\\chi_\\nu\\rangle\n\\tag{1.18}\n\\]\nOn the diagonal, \\(H_{\\mu\\mu}\\) measures the effective one-electron energy associated with atomic orbital \\(\\chi_\\mu\\). Off the diagonal, \\(H_{\\mu\\nu}\\) is the resonance or hopping integral: it measures how strongly an electron described on one atomic center can couple to the other. In the Wolfsberg-Helmholtz approximation one writes\n\\[\nH_{\\mu\\nu}\\approx \\tfrac12 K(H_{\\mu\\mu}+H_{\\nu\\nu})S_{\\mu\\nu}\n\\tag{1.19}\n\\]\nSo the interaction strength tracks the overlap. A positive overlap between like-phased lobes typically produces a negative interaction contribution, which is stabilizing for the bonding combination and destabilizing for the antibonding one.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.18",
        ...
        {
          "id": "01.03-p008",
          "text": "To determine the coefficients systematically, one inserts the LCAO expansion into the energy expression. If one writes \\( A=\\langle \\psi_i|\\psi_i\\rangle \\) and \\( B=\\langle \\psi_i|H^\\mathrm{eff}|\\psi_i\\rangle \\), then\n\\[\nA=\\sum_{\\mu,\\nu} c_{\\mu i}S_{\\mu\\nu}c_{\\nu i}\n\\tag{1.20}\n\\]\nand\n\\[\nB=\\sum_{\\mu,\\nu} c_{\\mu i}H_{\\mu\\nu}c_{\\nu i}\n\\tag{1.21}\n\\]\nBecause the overlap and Hamiltonian matrices are symmetric, \\(S_{\\mu\\nu}=S_{\\nu\\mu}\\) and \\(H_{\\mu\\nu}=H_{\\nu\\mu}\\), these quadratic forms really are the natural global measures of normalization and energy inside the chosen basis. The orbital energy is then the quotient\n\\[\ne_i=B/A\n\\tag{1.22}\n\\]\nAt this stage one does not yet have a solved eigenvalue problem; one has a variational problem over the coefficient vector.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.20",
        ...
        {
          "id": "01.03-p009",
          "text": "The variational principle says that the best coefficients are those for which small changes do not lower the energy any further. So one sets\n\\[\n\\frac{\\partial e_i}{\\partial c_{ki}}=0\n\\tag{1.23}\n\\]\nfor every coefficient \\(c_{ki}\\). Differentiating the quotient \\(e_i=B/A\\) gives\n\\[\n\\frac{\\partial B}{\\partial c_{ki}}-e_i\\frac{\\partial A}{\\partial c_{ki}}=0\n\\tag{1.25}\n\\]\nThe two derivatives are simple because \\(A\\) and \\(B\\) are quadratic:\n\\[\n\\frac{\\partial A}{\\partial c_{ki}}=2\\sum_\\mu S_{k\\mu}c_{\\mu i}\n\\tag{1.26}\n\\]\nand\n\\[\n\\frac{\\partial B}{\\partial c_{ki}}=2\\sum_\\mu H_{k\\mu}c_{\\mu i}\n\\tag{1.27}\n\\]\nSubstituting these into the stationarity condition and canceling the factor 2 yields\n\\[\n\\sum_\\mu (H_{k\\mu}-e_iS_{k\\mu})c_{\\mu i}=0\n\\tag{1.28}\n\\]\nThese are the secular equations: one equation for each basis-function index \\(k\\).",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.23",
            ...
            },
            {
              ...
              "spoken": "",
              "note": "Simplified variational condition for the coefficient \\(c_{ki}\\)."
            },
            {
              "id": "eq-1.26",
              "label": "1.26",
        ...
        {
          "id": "01.03-p010",
          "text": "A nontrivial set of coefficients exists only if this homogeneous system has zero determinant, so one must have\n\\[\n\\det(H-e_iS)=0\n\\tag{1.31}\n\\]\nSolving that polynomial equation gives \\(m\\) allowed orbital energies, one for each molecular orbital built from the \\(m\\) basis functions. After an energy \\(e_i\\) is found, the corresponding coefficients are recovered from the secular equations and then normalized by\n\\[\n\\sum_{\\mu,\\nu} c_{\\mu i}S_{\\mu\\nu}c_{\\nu i}=1\n\\tag{1.32}\n\\]\nWithin the chosen basis and the effective one-electron approximation, these energies are the variationally optimal ones. The formalism looks forbidding only because it is written at full generality. Its real content is simpler: choose a basis, measure its overlaps and interaction energies, and then find the linear combinations that make the effective one-electron energy stationary. Later, symmetry and perturbation theory will reduce many practical cases to the interaction of only two or three orbitals at a time.",
          "visuals": [],
          "equations": [
            {
              "id": "eq-1.31",
              ...
              "latex": "\\det(H-e_iS)=0",
              "spoken": "",
              "note": "Compact matrix form of the secular determinant condition required for a nontrivial solution of the homogeneous coefficient equations."
            },
            {
              "id": "eq-1.32",
              "label": "1.32",
