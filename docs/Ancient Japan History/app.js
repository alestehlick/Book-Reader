(() => {
  ...
  const DEFAULT_ILLUSTRATIONS_DIR = "illustrations";
  const DEFAULT_ILLUSTRATION_EXTENSIONS = ["webp", "jpg", "jpeg", "png"];
  ...
  const DEFAULT_MAP_RUNTIME_CONFIG = Object.freeze({
    assetBaseUrl: "",
  ...
  const state = {
    ...
    illustrationCatalog: new Map(),
    activeMapInstances: new Set(),
    activeMapObservers: new Set(),
    resumeSeconds: 0,
    lastPositionSave: 0,
  };
  ...
  function getMediaConfig() {
    ...
    const illustrationsDir = normalizePath(getDatasetValue("illustrationsDir"));
    const illustrationExtensions = parseDatasetList(getDatasetValue("illustrationExtensions"));
    ...
    return {
      ...
      illustrationsDir: illustrationsDir || DEFAULT_ILLUSTRATIONS_DIR,
      illustrationExtensions: illustrationExtensions.length > 0 ? illustrationExtensions : [...DEFAULT_ILLUSTRATION_EXTENSIONS],
  ...
  function getMapRuntimeConfig() {
    return {
      ...
      ...(window.ANCIENT_JAPAN_MAP_CONFIG || {}),
    };
  }
  ...
  function toAbsoluteUrl(path) {
    ...
    try {
      ...
      return absolute.replace(/%7B/gi, "{").replace(/%7D/gi, "}");
    } catch {
      return path;
    }
  }
  ...
  function loadStylesheetOnce(url) {
    ...
    return new Promise((resolve, reject) => {
      ...
      link.addEventListener("error", () => reject(new Error(`Could not load stylesheet: ${absolute}`)), { once: true });
  ...
  function loadScriptOnce(url, ready) {
    ...
    const absolute = toAbsoluteUrl(url);
    const existing = [...document.scripts].find((item) => item.src === absolute);
    if (existing) {
      return new Promise((resolve, reject) => {
        if (ready()) {
          resolve();
        ...
        existing.addEventListener("load", () => ready() ? resolve() : reject(new Error(`Runtime did not initialize: ${absolute}`)), { once: true });
        existing.addEventListener("error", () => reject(new Error(`Could not load script: ${absolute}`)), { once: true });
      });
    }
    return new Promise((resolve, reject) => {
      ...
      script.src = absolute;
      script.async = true;
      script.addEventListener("load", () => ready() ? resolve() : reject(new Error(`Runtime did not initialize: ${absolute}`)), { once: true });
      script.addEventListener("error", () => reject(new Error(`Could not load script: ${absolute}`)), { once: true });
  ...
  function ensureMapRuntime() {
    ...
    mapRuntimePromise = Promise.all([
      loadStylesheetOnce(config.maplibreCss),
      loadScriptOnce(config.maplibreJs, () => Boolean(window.maplibregl)),
      loadScriptOnce(config.pmtilesJs, () => Boolean(window.pmtiles?.Protocol)),
    ]).then(() => {
      if (!mapProtocol) {
        mapProtocol = new window.pmtiles.Protocol();
        window.maplibregl.addProtocol("pmtiles", mapProtocol.tile);
      }
      return { maplibregl: window.maplibregl, config };
    }).catch((error) => {
  ...
  function replaceStyleTokens(value, tokens) {
    if (Array.isArray(value)) {
      return value.map((item) => replaceStyleTokens(item, tokens));
    }
    if (value && typeof value === "object") {
      return Object.fromEntries(
        Object.entries(value).map(([key, item]) => [key, replaceStyleTokens(item, tokens)])
      );
    }
  ...
  async function loadInteractiveMapStyle(mapData) {
    ...
    const impossiblePeriod = "__period_not_used__";
    const tokens = {
      ...
      __SITE_PERIOD_2__: String(periods[1] || impossiblePeriod),
      __SITE_PERIOD_3__: String(periods[2] || impossiblePeriod),
      __ACCENT__: String(mapData.accent || "#80634E"),
      __MAP_DATE__: Number.isFinite(Number(mapData.date)) ? Number(mapData.date) : -100000,
    };
    return replaceStyleTokens(style, tokens);
  }

  function destroyInteractiveMaps() {
    state.activeMapObservers.forEach((observer) => observer.disconnect());
    state.activeMapObservers.clear();
    state.activeMapInstances.forEach((map) => {
      try {
        map.remove();
      } catch {
        // A partially initialized WebGL map can already have released itself.
      }
    });
    state.activeMapInstances.clear();
  }
  ...
  function normalizeParagraph(paragraph, section, paragraphIndex) {
    ...
    return {
      ...
      illustrationRefs: normalizeMediaList(paragraph?.illustrationRefs),
      genealogy,
      notes,
  ...
  function indexBookData(book) {
    ...
    const illustrationCatalogEntries = normalizeMediaList(book?.catalogs?.illustrations);
    ...
    state.illustrationCatalog = new Map(
      illustrationCatalogEntries.filter((item) => item?.id).map((item) => [String(item.id), item])
    );
  ...
  function normalizeIllustrationReference(rawRef) {
    if (typeof rawRef === "string") {
      const id = rawRef.trim();
      return id ? { id, role: "supporting" } : null;
    }
    ...
    return {
      ...rawRef,
      id,
      role: String(rawRef.role || "supporting").trim().toLowerCase(),
    };
  }

  function resolveIllustrationEntry(rawRef, index) {
    const ref = normalizeIllustrationReference(rawRef);
    if (!ref) return null;
    const asset = state.illustrationCatalog.get(ref.id);
    if (!asset || asset.status !== "approved") return null;

    const explicitPath = normalizePath(asset.src || "");
    const srcCandidates = explicitPath
      ? resolveNamedMediaSources(explicitPath, MEDIA_CONFIG.illustrationsDir, MEDIA_CONFIG.illustrationExtensions)
      : [];
    if (asset.fallbackSrc) srcCandidates.push(normalizePath(asset.fallbackSrc));
    ...
    return {
      ...
      role: ref.role,
      src: srcCandidates[0] || "",
      srcCandidates: uniqueStrings(srcCandidates),
      alt: String(asset.alt || `Illustration ${index + 1}`).trim(),
  ...
  function resolveMapEntry(rawEntry, index) {
    ...
    if (typeof rawEntry === "object") {
      const type = String(rawEntry.type || "image").trim().toLowerCase();
      const label = String(rawEntry.label || rawEntry.title || rawEntry.name || "").trim();
      const caption = String(rawEntry.caption || rawEntry.description || "").trim();
  ...
  function dedupeResolvedMaps(entries) {
    ...
    return entries.filter((item) => {
      if (!item || (!item.src && item.type !== "interactive")) return false;
      if (item.type === "interactive") {
        const viewKey = JSON.stringify(item.view || {});
        const key = `interactive::${item.archive || ""}::${item.plate || ""}::${viewKey}`;
        if (seen.has(key)) return false;
        seen.add(key);
        return true;
      }
  ...
  function getEffectiveIllustrationEntries(paragraph) {
    const refs = normalizeMediaList(paragraph?.illustrationRefs)
      .map((entry, index) => resolveIllustrationEntry(entry, index))
  ...
  function getEffectiveMapEntries(paragraph) {
    ...
    return dedupeResolvedMaps(
      rawEntries
        ...
        .filter((item) => item && (item.src || item.type === "interactive"))
    );
  }
  ...
  function illustrationStatusLabel(item) {
    const labels = {
      "contemporary-evidence": "Contemporary evidence",
      "archaeological-evidence": "Archaeological evidence",
      "documented-landscape": "Documented landscape",
      "scholarly-reconstruction": "Scholarly reconstruction",
      "ai-reconstruction": "AI reconstruction",
      "later-depiction": "Later depiction",
      "mythic-interpretation": "Mythic interpretation",
    };
    return labels[item?.historicalStatus] || (item?.generated ? "AI reconstruction" : "Documented image");
  }

  function appendIllustrationDetails(card, item) {
    const footer = document.createElement("div");
    footer.className = "illustration-footer";

    const badge = document.createElement("span");
    badge.className = `evidence-badge evidence-badge--${item.evidenceLevel || "documented"}`;
    badge.textContent = illustrationStatusLabel(item);
    footer.appendChild(badge);

    const details = document.createElement("details");
    details.className = "illustration-details";
    ...
    if (evidence.brief) {
      ...
      brief.textContent = evidence.brief;
      body.appendChild(brief);
    }
    if (Array.isArray(evidence.uncertainties) && evidence.uncertainties.length > 0) {
      const uncertainty = document.createElement("p");
      uncertainty.textContent = `Uncertainty: ${evidence.uncertainties.join(" ")}`;
      body.appendChild(uncertainty);
    }

    const rights = item.rights && typeof item.rights === "object" ? item.rights : {};
    const creditText = [rights.credit, rights.institution, rights.license].filter(Boolean).join(" · ");
    if (creditText) {
      const credit = document.createElement("p");
      credit.className = "illustration-credit";
      credit.textContent = creditText;
      body.appendChild(credit);
    }
    if (item.sourceUrl) {
      const source = document.createElement("a");
      source.className = "illustration-source-link";
      source.href = toSafeUrl(item.sourceUrl);
  ...
  function renderIllustrations(entries) {
    if (entries.length === 0) return;
    const rail = document.createElement("div");
    rail.className = `illustration-rail${entries.length === 1 ? " is-single" : ""}`;
    rail.setAttribute("aria-label", "Paragraph illustrations");

    entries.forEach((item, index) => {
      const card = document.createElement("article");
      card.className = `illustration-card${item.role === "primary" ? " is-primary" : ""}`;
      ...
      img.setAttribute("aria-label", `Enlarge illustration ${index + 1}`);

      const candidates = uniqueStrings(item.srcCandidates?.length ? item.srcCandidates : [item.src]);
      let candidateIndex = 0;
      const loadCandidate = () => {
        img.src = toSafeUrl(candidates[candidateIndex] || item.src);
      };
      img.addEventListener("error", () => {
        ...
        card.replaceWith(createMissingMediaCard(`Could not load illustration file: ${item.src}`));
      });
    ...
    paragraphFiguresEl.appendChild(rail);
  }
  ...
  function formatMediaSummary(illustrationCount, figureCount, videoCount) {
    const parts = [];

    if (illustrationCount > 0) {
      parts.push(`${illustrationCount} illustration${illustrationCount === 1 ? "" : "s"}`);
    }
  ...
  function openImageViewer(src, alt) {
    ...
    if (!dialog) {
      ...
      dialog.innerHTML = `<button type="button" class="image-viewer-close" aria-label="Close enlarged image">×</button><img alt="" />`;
  ...
  function createMapPopupContent(properties) {
    const content = document.createElement("div");
    content.className = "atlas-popup-content";
    const name = properties.map_label || properties.name_en || properties.name || "Map feature";
    const title = document.createElement("div");
    title.className = "atlas-popup-name";
    ...
    layers.forEach((layerId) => {
      ...
      map.on("click", layerId, (event) => {
        ...
        new maplibregl.Popup({ closeButton: true, closeOnClick: true, maxWidth: "290px" })
          ...
          .setDOMContent(createMapPopupContent(feature.properties || {}))
          .addTo(map);
      });
    });
  }
    ...
    const failToFallback = (error) => {
      ...
      if (map) {
        ...
        try {
          ...
          // The runtime may have already released a failed WebGL context.
        }
      }
      if (wrapper.isConnected) wrapper.replaceWith(createStaticMapCard(mapData, index));
    };

    try {
      const [{ maplibregl, config }, style] = await Promise.all([
        ensureMapRuntime(),
        loadInteractiveMapStyle(mapData),
      ]);
      ...
      map = new maplibregl.Map({
        ...
        pixelRatio: Math.min(window.devicePixelRatio || 1, Number(config.maxPixelRatio) || 2),
      });
      ...
      const resetView = (duration = 0) => {
        if (Array.isArray(bounds) && bounds.length === 2) {
          map.fitBounds(bounds, { padding: 34, duration, maxZoom });
        }
      };
      reset.addEventListener("click", () => resetView(320));

      const loadTimeout = window.setTimeout(() => {
        if (!map?.loaded()) failToFallback(new Error("Interactive map timed out while loading."));
      }, 18000);

      map.once("load", () => {
        window.clearTimeout(loadTimeout);
        if (!wrapper.isConnected) return;
        ["ritsuryo-provinces-fill", "ritsuryo-provinces-line", "ritsuryo-province-labels"].forEach((layerId) => {
          if (map.getLayer(layerId)) {
            map.setLayoutProperty(layerId, "visibility", mapData.showProvinces ? "visible" : "none");
          }
        });
        resetView(0);
  ...
  function createInteractiveMapCard(mapData, index) {
    ...
    canvas.className = "interactive-map-canvas";
    canvas.setAttribute("role", "region");
    canvas.setAttribute("aria-label", mapData.alt || `Interactive map ${index + 1}`);
    canvas.setAttribute("aria-busy", "true");
    const status = document.createElement("div");
    status.className = "interactive-map-status";
    status.setAttribute("role", "status");
    status.textContent = "Preparing scalable map…";
    const reset = document.createElement("button");
    reset.className = "interactive-map-reset";
    reset.type = "button";
    reset.hidden = true;
    reset.textContent = "↺";
    ...
    if ("IntersectionObserver" in window) {
      const observer = new IntersectionObserver((entries) => {
        ...
        observer.disconnect();
        state.activeMapObservers.delete(observer);
        start();
      }, { rootMargin: "500px 0px" });
      state.activeMapObservers.add(observer);
  ...
  function renderMaps(entries) {
    ...
    entries.forEach((mapData, index) => {
      if (!mapData) return;
      const card = mapData.type === "interactive"
        ? createInteractiveMapCard(mapData, index)
        : createStaticMapCard(mapData, index);
      allMapsEl.appendChild(card);
    });
  }
  ...
  function renderFigureMedia() {
    ...
    figuresSummaryEl.textContent = formatMediaSummary(illustrationCount, figureCount, videoCount);

    renderIllustrations(effectiveIllustrations);
  ...
  function applyFiguresVisibility(persist = false) {
    ...
    if (figuresToggleBtn) {
      ...
      figuresToggleBtn.setAttribute(
        "aria-label",
        state.figuresVisible ? "Hide illustrations" : "Show illustrations"
      );
